...
const fillproduct=()=>{
  ...
  const handleIconSave = async () => {
    ...
    var result = await postDataImage("productimages/editicon", formData);
    if (result.result) {
      Swal.fire({
        position: "top-start",
        icon: "success",
  ...
  const handleDelete = async (rowData) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        var dresult = await postData("productimages/deletedata", { productimageid: rowData.productimageid });
        if (dresult.result) {
      ...
      <MaterialTable
        ...
        columns={[
          ...
          { title: "Product Name", field: "productname" },
          {
            title: "Icon",
            field: "icon",
            render: (rowData) => <img src={`${ServerURL}/images/${rowData.image}`} style={{ width: 150, borderRadius: "10%" }} alt="Product Image" />,
          },
        ]}
        data={list}
        actions={[
